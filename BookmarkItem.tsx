...
const BookmarkItem: React.FC<BookmarkItemProps> = ({ bookmark, layout, onInfo, onDelete, onToggleFavorite, onBookmarkVisit, category, folder, labels, isReadingListItem, onMarkAsUnread, isSelected = false, onToggleSelect, onShowToast }) => {
        ...
        {onToggleSelect && !isPrivateCollectionItem && (
            <div className="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity" style={isSelected ? {opacity: 1} : {}}>
              <input 
                  ...
                  className="h-5 w-5 rounded border-[var(--border-primary)] text-[var(--accent-primary)] focus:ring-[var(--accent-primary)] bg-[var(--bg-tertiary)]"
      ...
      {onToggleSelect && !isPrivateCollectionItem && (
          <div className="mr-3 flex-shrink-0">
            <input 
                ...
                checked={isSelected} 
                onChange={() => onToggleSelect(bookmark.id)}
                className="h-5 w-5 rounded border-[var(--border-primary)] bg-[var(--bg-tertiary)] text-[var(--accent-primary)] focus:ring-[var(--accent-primary)]"
                onClick={(e) => e.stopPropagation()}
            />
          </div>
      )}
