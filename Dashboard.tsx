...
function Dashboard({ user: loggedInUser, setUser, theme, setTheme, layout, setLayout, font, setFont, onLogout, onPrivateFolderClick, updateUserName }: DashboardProps) {
  const { data: bookmarks, add: addBookmark, update: updateBookmark, remove: removeBookmark, bulkUpdate: bulkUpdateBookmarks, bulkDelete: bulkDeleteBookmarks, loading: bookmarksLoading } = useFirestore<Bookmark>('bookmarks', loggedInUser.id);
  ...
  const [selectedMainMenuItem, setSelectedMainMenuItem] = useState<'My bookmarks' | 'Locker'>('My bookmarks');
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300">
      <div className={`fixed inset-y-0 left-0 z-40 w-72 transform transition-transform duration-300 ease-in-out ${isSidebarOpenOnMobile ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <Sidebar
          ...
          selectedOption={selectedMainMenuItem}
          onSelectOption={(option) => setSelectedMainMenuItem(option as 'My bookmarks' | 'Locker')}
        />
      </div>
      ...
      <main className={`h-screen overflow-y-auto md:ml-72 ${isAnyModalOpen ? 'overflow-hidden' : ''}`}>
        <div className="sticky top-0 z-20">
          <Header
            ...
            onSelectMainMenuItem={setSelectedMainMenuItem} 
            activeMainMenuItem={selectedMainMenuItem}
          />
          {view === 'dashboard' ? (
            <DashboardHeader
              activeFilter={activeFilter}
